...
1. **The go-green moment.** Replaced AvailRow's two inconsistent entry points (a small off-state chip vs. a full on-state CTA) with **one control**. Going grey stays a lighter tap + confirm-sheet, deliberately, since ending your status should be easier to reverse than it was to start (never harder to undo than to commit to).
   - **Revised 2026-08-02 (R22): that control is your own tile in the rail, not a swipe.** The swipe is deleted. It had been justified as *"a deliberate physical gesture makes accidental green-status essentially impossible"* — which was never quite true of the shipped build: **the slide only ever opened the Go Green sheet, and the sheet's "I'm free" button was always the commit.** The protection was the sheet, not the drag, so replacing the drag with a tap costs nothing. What it buys: one surface at the top of the feed instead of two competing for it, and the same gesture teaching both objects (tap your own face to go green, tap the cow to plan a Moove).
   - **Revised again 2026-08-04 (R24): the tile is a traffic light, not your face.** Two problems with the face, both found on the shipped build. Your own photo is the least useful image on the screen and it was spending the rail's most valuable slot saying nothing; and a `purple-500` `+` badge on a circular photo is, everywhere else on a phone, *add a story* — the exact wrong promise for the app's most important action. A traffic light is the only object on the rail that obviously **is** a control, and its three lamps carry the whole model without a word: there is a green, you are not currently on it, tapping puts you there. Going green then swaps the light for your face, so you appear in the rail once and exactly like everybody else. The symmetry R23 built survives, restated: **tap the light, tap the cow** — two objects rather than one object and one person, which is the clearer pair. Label is **"Go green"** app-wide; "Go free" is retired, because one action with two names is what made the old tile ambiguous in the first place.
