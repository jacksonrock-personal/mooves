...
- **In-app messaging: one bounded exception, and only one.** Moove comments (Phase 21) are permitted. Nothing else is — no DMs, no per-person message surface, no inbox, no threads, no unread counts or badges, no realtime message delivery. The exception holds only while all four walls hold: comments live **only** on a Moove, **only** for people who joined it, are **never shown or hinted at** to anyone who hasn't, and **die when the Moove does**. Text on a green, text between two people, text that outlives its Moove, or a count shown to a non-joiner are violations of this rule — not extensions of the exception.
  - **Wall 2 amended twice, in daylight (2026-07-28).** R8 added likes + @tagging inside the roster. **R11 then allowed tagging someone who has NOT joined — bounded to your own friends who can ALREADY see the Moove in their feed.** A tag can therefore never be what *discloses* a Moove; it only converts "sitting in a feed" into "somebody asked me". **Wall 3 bends exactly this far and no further:** a tagged non-joiner gets one push that names the tagger, and it lands on the Moove **card** with "I'm in" — they still cannot read the thread, see a count, or learn what was said. Anything wider (a mention inbox, letting a tagged outsider read comments, tagging someone who cannot see the Moove) is a violation, not an extension.
