...
export default function Rail({ people, seed, onOpenMine, onText }: RailProps) {
  return (
    ...
    <div className="flex gap-[11px] overflow-x-auto pt-0.5 pb-1 mb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        const inner = (
          <>
            <span className="relative">
              ...
              {p.isGreen ? (
                <span
                  className={`absolute -inset-1 rounded-full pointer-events-none border-[2.5px] ${
                    later ? 'border-green-500/40' : 'border-green-500'
              ...
              {/* R22 — the + is yours alone, and only while you are grey. On a
                  ...
                  lives. */}
              {p.isMe && !p.isGreen ? (
                <span className="absolute -right-[3px] -bottom-[3px] w-[22px] h-[22px] rounded-full bg-purple-500 border-[2.5px] border-purple-50 flex items-center justify-center shadow-[0_2px_6px_rgba(124,92,219,0.42)]">
