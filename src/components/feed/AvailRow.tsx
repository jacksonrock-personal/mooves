...
// The entire off-state row is the tap target (the "Go free" pill is a visual cue).
...
export default function AvailRow({ isAvailable, onToggle }: AvailRowProps) {
  ...
  return (
    <button
      onClick={onToggle}
      aria-label="Go free"
      className="w-full flex items-center justify-between py-[14px] px-5 rounded-2xl bg-card-white border-[1.5px] border-[#E8E4F5] mb-5 text-left"
    >
      ...
      <span className="flex items-center gap-1.5 rounded-full bg-status-green px-3.5 py-1.5">
